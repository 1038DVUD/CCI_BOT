...
symbol = 'KRW-EGLD'  # 멀티버스엑스 (EGLD) / 한국 원화 (KRW)
...
while True:           
    ...
    if current_time.tm_min % 5 == 4 and current_time.tm_sec == 50:
        try:
            # 최근 데이터 불러오기
            ...
            # 매도 조건
            if previous_cci >= 100 and current_cci <= 100 and buy_order_executed > 0:
                current_price = pyupbit.get_current_price(symbol)  # current_price 정의
                slack_client.chat_postMessage(
                    channel='#stock_cci진입',
                    text=f"매매종목: {symbol}, \n이전 CCI {previous_cci}로 과매수 과열구간, \n현재 CCI {current_cci}로 100 하향돌파")

                profit_rate = ((current_price - buy_price) / buy_price) * 100
                
                if profit_rate >= 0.1:  # 현재 수익률이 0.1% 이상일 때만 매도 주문 실행
                    upbit.sell_market_order(symbol, upbit.get_balance(symbol))
                    message = f"매매종목: {symbol}, \n매도 주문 실행 - CCI: {current_cci}, \n수익률: {profit_rate:.2f}%"
