...
symbol = 'KRW-BTC'  # 멀티버스엑스 (EGLD) / 한국 원화 (KRW)
# CCI 계산을 위한 데이터 차트 시간 간격과 기간 설정 (15분봉)
interval = 'minute5'
cci_period = 3  # timeperiod 값을 더 작게 설정
...
while True:
    ...
    if current_time.tm_min % 5 == 0:
        try: 
            # 최근 데이터 불러오기
            df = pyupbit.get_ohlcv(symbol, interval=interval, count=cci_period + 3)  # +1 추가하여 데이터 확보
    ...
    text=f"내역: {cci}, {df}")
            ...
            if previous_cci < -100 and current_cci > -100 and krw_balance > 6000 and buy_order_executed < 3 and (last_buy_time is None or (current_time - last_buy_time) >= 600): # 원래값 1800
                ...
                buy_price = pyupbit.get_current_price(symbol)  # 매수 가격 업데이트
            ...
            if previous_cci >= 100 and current_cci <= 100:
                ...
                if buy_price is not None:  # buy_price가 None이 아닌 경우에만 매도 주문 실행
